...
视觉必须继续使用 Execute 全局 Token，不为 Profile 建立独立黑黄主题。顶部账户区域允许使用与 Today 一致的浅蓝紫渐变和轻量状态 Pill；设置区通过留白、轻边框、圆角与语义色图标建立层级，避免连续全宽 Divider、大面积深色块和过量独立 Card。

“关于”页提供 Android 手动“检查更新”：只有用户点击后才访问 GitHub Release；发现新版本后必须再次确认才下载，下载完成后交给 Android 系统安装器确认。禁止启动自动检查、后台自动下载和静默安装。
