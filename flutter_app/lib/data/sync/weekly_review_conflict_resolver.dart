...
class WeeklyReviewConflictResolver {
  WeeklyReviewConflictResolver(this.database, {Uuid? uuid})
  ...
  Future<void> keepServer(String conflictId) async {
    await database.transaction(() async {
      ...
      if (conflict.entityType != DriftWeeklyReviewRepository.entityType) {
        throw StateError('仅支持处理周复盘冲突');
      }
      ...
      if (conflict.serverDeleted) {
        ...
        if (payload == null || version == null) {
          throw StateError('云端 WeeklyReview 冲突缺少内容');
        }
  ...
  Future<void> keepLocal({
    ...
    await database.transaction(() async {
      ...
      if (version == null || version.isEmpty) {
        throw StateError('周复盘冲突缺少最新云端版本');
      }
      ...
      final queued = await (database.select(database.syncOutbox)
            ...
            ..limit(1))
          .getSingleOrNull();
      if (queued == null) throw StateError('周复盘冲突缺少本地待同步变更');
      ...
      if (queued.operation == 'delete') {
        ...
        final row = await (database.select(database.weeklyReviews)
            ...
            .getSingleOrNull();
        if (row == null) throw StateError('本地 WeeklyReview 已不存在');

        final local = WeeklyReviewDatabaseMapper.fromRow(row).copyWith(
          updatedAt: now,
        ...
        await database
            .into(database.weeklyReviews)
            .insertOnConflictUpdate(WeeklyReviewDatabaseMapper.toRow(local));
        await database.into(database.syncOutbox).insert(
              db.SyncOutboxCompanion.insert(
                changeId: _uuid.v4(),
                userId: conflict.userId,
                ...
                clientModifiedAt: now,
                payloadJson: Value(
                  jsonEncode(WeeklyReviewWireMapper.toPayload(local)),
                ),
                createdAt: now,
              ),
            );
  ...
  Future<void> _deleteReview(String userId, String reviewId) =>
      (database.delete(database.weeklyReviews)
            ..where(
              (table) =>
  ...
  static Map<String, dynamic> _decodeObject(String raw) {
    ...
    throw const FormatException('WeeklyReview conflict payload must be object');
  }
}
